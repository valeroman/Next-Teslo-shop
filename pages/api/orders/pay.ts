...
const  payOrder = async(req: NextApiRequest, res: NextApiResponse<Data>) => {
    ...
    if ( !paypalBearerToken ) {
        return res.status(400).json({ message: 'No se pudo confitmar el token de paypal' })
    }
    ...
    dbOrder.isPaid = true;
    await dbOrder.save();
    // aqui se podria crearfuncion para notificar por correo que fue pagada la orden
    await db.disconnect();
